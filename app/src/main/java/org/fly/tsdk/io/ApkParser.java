...
public final class ApkParser implements Closeable {
        ...
        try (ZipFile zipfile = new ZipFile(apkFile)) {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (entryName.equalsIgnoreCase(filename)) {
                    ...
                    if (size > 0) {
                        Source source = Okio.source(zipfile.getInputStream(entry));
                        return IoUtils.readBytes(source);
                    }
    ...
    public long saveToFile(String filename, File outFile) {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (entryName.equalsIgnoreCase(filename)) {
                    ...
                    if (size > 0) {
                        ...
                        return IoUtils.write(outFile, source);
                    }
                }
            }
        } catch (IOException e) {
    ...
    public ByteBuffer getComment() throws IOException {
        final int commentLength = (int) getCommentLength();

        final ByteBuffer commentBuffer = ByteBuffer.allocate(commentLength);
        if (commentLength == 0)
            return commentBuffer;

        fileChannel.position(fileChannel.size() - commentLength);
        fileChannel.read(commentBuffer);

       return commentBuffer;
    }

    public long getCommentLength() throws IOException {
    ...
    public long findCentralDirStartOffset() throws IOException {
        return findCentralDirStartOffset(getCommentLength());
    }

    public long findCentralDirStartOffset(final long commentLength) throws IOException {
        // End of central directory record (EOCD)
        // Offset    Bytes     Description[23]
        // 0           4       End of central directory signature = 0x06054b50
        // 4           2       Number of this disk
    ...
    public Pair<ByteBuffer, Long> findApkSigningBlock() throws IOException, SignatureNotFoundException {
        return findApkSigningBlock(findCentralDirStartOffset());
    }

    public Pair<ByteBuffer, Long> findApkSigningBlock(final long centralDirOffset) throws IOException, SignatureNotFoundException {
    ...
    public Map<Integer, ByteBuffer> findSigningBlockIdValues() throws SignatureNotFoundException {
        try {
    ...
    public Map<Integer, ByteBuffer> findSigningBlockIdValues(final ByteBuffer apkSigningBlock) throws SignatureNotFoundException {
     ...
     * buffer's byte order.
     */
    private ByteBuffer sliceFromTo(final ByteBuffer source, final int start, final int end) {
        if (start < 0) {
     ...
     * {@code size}.
     */
    private ByteBuffer getByteBuffer(final ByteBuffer source, final int size)
    ...
    private void checkByteOrderLittleEndian(final ByteBuffer buffer) {
